...
vi.mock('../../config.js', () => ({
  readGlobalConfig: vi.fn(async () => ({ registry: 'https://clawdsquad.com', token: 'tkn' })),
}))

const mockGetRegistry = vi.fn(async (_opts: unknown, _params?: unknown) => 'https://clawdsquad.com')
vi.mock('../registry.js', () => ({
...
async function makeTmpWorkdir() {
  const root = await mkdtemp(join(tmpdir(), 'clawdsquad-publish-'))
  return root
}
