...
describe('registry resolution', () => {
  it('prefers explicit registry over discovery/cache', async () => {
    readGlobalConfig.mockResolvedValue({ registry: 'https://auth.clawdsquad.com' })
    discoverRegistryFromSite.mockResolvedValue({ apiBase: 'https://clawdsquad.com' })
  ...
  it('ignores legacy registry and updates cache from discovery', async () => {
    readGlobalConfig.mockResolvedValue({ registry: 'https://auth.clawdsquad.com', token: 'tkn' })
    discoverRegistryFromSite.mockResolvedValue({ apiBase: 'https://clawdsquad.com' })

    const registry = await getRegistry(makeOpts(), { cache: true })

    expect(registry).toBe('https://clawdsquad.com')
