...
export function getGlobalConfigPath() {
  ...
  if (process.platform === 'darwin') {
    return join(home, 'Library', 'Application Support', 'clawdsquad', 'config.json')
  }
  const xdg = process.env.XDG_CONFIG_HOME
  if (xdg) return join(xdg, 'clawdsquad', 'config.json')
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA
    if (appData) return join(appData, 'clawdsquad', 'config.json')
  }
  return join(home, '.config', 'clawdsquad', 'config.json')
}
