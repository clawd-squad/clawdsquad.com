...
### Added
- Telemetry: track installs via `clawdsquad sync` (logged-in only), per root, with 120-day staleness.
...
- CLI: ignore legacy `auth.clawdsquad.com` registry and prefer site discovery.
...
- CLI package: bundle schema internally (no external `clawdsquad-schema` publish).
- Repo: mark `clawdsquad-schema` as private to prevent publishing.
