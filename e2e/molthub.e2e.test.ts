...
import {
  ...
  parseArk,
} from 'clawdsquad-schema'
import { Agent, setGlobalDispatcher } from 'undici'
import { describe, expect, it } from 'vitest'
import { readGlobalConfig } from '../packages/clawdsquad/src/config'
...
async function makeTempConfig(registry: string, token: string | null) {
  const dir = await mkdtemp(join(tmpdir(), 'clawdsquad-e2e-'))
...
describe('clawdsquad e2e', () => {
  it('prints CLI version via --cli-version', async () => {
    const result = spawnSync('bun', ['clawdsquad', '--cli-version'], {
      cwd: process.cwd(),
      encoding: 'utf8',
    })
  ...
  it('search endpoint returns a results array (schema parse)', async () => {
    const registry = process.env.MOLTHUB_REGISTRY?.trim() || 'https://clawdsquad.com'
  ...
  it('cli search does not error on multi-result responses', async () => {
    const registry = process.env.MOLTHUB_REGISTRY?.trim() || 'https://clawdsquad.com'
    const site = process.env.MOLTHUB_SITE?.trim() || 'https://clawdsquad.com'
    const token = mustGetToken() ?? (await readGlobalConfig())?.token ?? null

    const cfg = await makeTempConfig(registry, token)
    try {
      const workdir = await mkdtemp(join(tmpdir(), 'clawdsquad-e2e-workdir-'))
  ...
  it('assumes a logged-in user (whoami succeeds)', async () => {
    ...
    const token = mustGetToken() ?? (await readGlobalConfig())?.token ?? null
    if (!token) {
      throw new Error('Missing token. Set MOLTHUB_E2E_TOKEN or run: bun clawdsquad auth login')
    }
    ...
    try {
      ...
      const result = spawnSync(
        'bun',
        ['clawdsquad', 'whoami', '--site', site, '--registry', registry],
        {
          cwd: process.cwd(),
          env: { ...process.env, MOLTHUB_CONFIG_PATH: cfg.path, MOLTHUB_DISABLE_TELEMETRY: '1' },
          encoding: 'utf8',
  ...
  it('sync dry-run finds skills from an explicit root', async () => {
    ...
    const cfg = await makeTempConfig(registry, token)
    const root = await mkdtemp(join(tmpdir(), 'clawdsquad-e2e-sync-'))
    try {
  ...
  it('sync dry-run finds skills from moltbot.json roots', async () => {
    ...
    const cfg = await makeTempConfig(registry, token)
    const root = await mkdtemp(join(tmpdir(), 'clawdsquad-e2e-moltbot-'))
    ...
    try {
      ...
      const result = spawnSync(
        'bun',
        ['clawdsquad', 'sync', '--dry-run', '--all', '--site', site, '--registry', registry],
        {
          cwd: process.cwd(),
          env: {
            ...process.env,
  ...
  it('publishes, deletes, and undeletes a skill (logged-in)', async () => {
    ...
    const cfg = await makeTempConfig(registry, token)
    const workdir = await mkdtemp(join(tmpdir(), 'clawdsquad-e2e-publish-'))
    const installWorkdir = await mkdtemp(join(tmpdir(), 'clawdsquad-e2e-install-'))
    ...
    try {
      ...
      const list = spawnSync(
        'bun',
        ['clawdsquad', 'list', '--site', site, '--registry', registry, '--workdir', installWorkdir],
        {
          cwd: process.cwd(),
          env: { ...process.env, MOLTHUB_CONFIG_PATH: cfg.path, MOLTHUB_DISABLE_TELEMETRY: '1' },
          encoding: 'utf8',
