...
export function getSiteName(mode: SiteMode = getSiteMode()) {
  return mode === 'souls' ? 'SoulHub' : 'ClawdSquad'
}
...
export function getSiteDescription(mode: SiteMode = getSiteMode()) {
  return mode === 'souls'
    ...
    : 'ClawdSquad — a fast skill registry for agents, with vector search.'
}

export function getSiteUrlForMode(mode: SiteMode = getSiteMode()) {
  return mode === 'souls' ? getOnlyCrabsSiteUrl() : getClawdSquadSiteUrl()
}
