...
describe('site helpers', () => {
  it('returns default and env configured site URLs', () => {
    expect(getClawdSquadSiteUrl()).toBe('https://clawdsquad.com')
    withMetaEnv({ VITE_SITE_URL: 'https://example.com' }, () => {
      expect(getClawdSquadSiteUrl()).toBe('https://example.com')
    })
  })
  ...
  it('detects site mode from host and URLs', () => {
    ...
    withMetaEnv({ VITE_SOULHUB_HOST: 'souls.example.com' }, () => {
      ...
      expect(detectSiteMode('clawdsquad.com')).toBe('skills')
      ...
      expect(detectSiteModeFromUrl('https://clawdsquad.com')).toBe('skills')
    })
  })
  ...
  it('derives site metadata from mode', () => {
    expect(getSiteName('skills')).toBe('ClawdSquad')
    expect(getSiteName('souls')).toBe('SoulHub')

    expect(getSiteDescription('skills')).toContain('ClawdSquad')
    expect(getSiteDescription('souls')).toContain('SoulHub')

    expect(getSiteUrlForMode('skills')).toBe('https://clawdsquad.com')
