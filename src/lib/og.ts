import { getClawdSquadSiteUrl, getOnlyCrabsSiteUrl } from './site'
...
const DEFAULT_DESCRIPTION = 'ClawdSquad — a fast skill registry for agents, with vector search.'
...
export function buildSkillMeta(source: SkillMetaSource): SkillMeta {
  ...
  const title = `${displayName} — ClawdSquad`
  const description =
    summary || (owner ? `Agent skill by @${owner} on ClawdSquad.` : DEFAULT_DESCRIPTION)
