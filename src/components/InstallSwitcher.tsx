...
export function InstallSwitcher({ exampleSlug = 'sonoscli' }: InstallSwitcherProps) {
  ...
  const command = useMemo(() => {
    switch (pm) {
      case 'npm':
        return `npx clawdsquad@latest install ${exampleSlug}`
      case 'pnpm':
        return `pnpm dlx clawdsquad@latest install ${exampleSlug}`
      case 'bun':
        return `bunx clawdsquad@latest install ${exampleSlug}`
    }
  }, [exampleSlug, pm])
