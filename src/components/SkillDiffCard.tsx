...
function getMonacoThemeName() {
  if (typeof document === 'undefined') return 'clawdsquad-light'
  return document.documentElement.dataset.theme === 'dark' ? 'clawdsquad-dark' : 'clawdsquad-light'
}
...
function applyMonacoTheme(monaco: NonNullable<ReturnType<typeof useMonaco>>) {
  ...
  monaco.editor.defineTheme(`clawdsquad-${isDark ? 'dark' : 'light'}`, {
    base,
    inherit: true,
    rules: [
  ...
  monaco.editor.setTheme(`clawdsquad-${isDark ? 'dark' : 'light'}`)
}
