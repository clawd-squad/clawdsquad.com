...
Base URL: `https://clawdsquad.com` (default).
...
## Registry discovery (`/.well-known/clawdsquad.json`)

The CLI can discover registry/auth settings from the site:

- `/.well-known/clawdsquad.json` (JSON)

Schema:

```json
{ "apiBase": "https://clawdsquad.com", "authBase": "https://clawdsquad.com", "minCliVersion": "0.0.5" }
```
