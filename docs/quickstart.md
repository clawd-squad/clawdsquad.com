...
```bash
bun clawdsquad install <slug> --workdir /tmp/clawdsquad-demo --dir skills
```

Update:

```bash
bun clawdsquad update --all
```
...
```bash
mkdir -p /tmp/clawdsquad-skill-demo && cd /tmp/clawdsquad-skill-demo
cat > SKILL.md <<'EOF'
---
name: Demo Skill
...
```bash
bun clawdsquad publish . \
  --slug clawdsquad-demo-$(date +%s) \
  --name "Demo $(date +%s)" \
  --version 1.0.0 \
  --tags latest \
...
```bash
bun clawdsquad sync --all --dry-run --no-input
```
