...
## `clawdsquad login` opens browser but never completes
...
```bash
clawdsquad sync --root /path/to/skills
```
...
- Options:
  ...
  - overwrite: `clawdsquad update <slug> --force`
  - publish as fork: copy to new folder/slug then `clawdsquad publish ... --fork-of upstream@version`
