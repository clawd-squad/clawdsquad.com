...
CLI package: `packages/clawdsquad/` (bin: `clawdsquad`).
...
- `--site <url>`: base URL for browser login (default: `https://clawdsquad.com`)
- `--registry <url>`: API base URL (default: discovered, else `https://clawdsquad.com`)
...
- macOS: `~/Library/Application Support/clawdsquad/config.json`
...
- Headless: `clawdsquad login --token clh_...`
...
- Writes:
  - `<workdir>/.clawdsquad/lock.json`
  - `<skill>/.clawdsquad/origin.json`

### `list`

- Reads `<workdir>/.clawdsquad/lock.json`.
