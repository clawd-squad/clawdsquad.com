...
- CLI: `packages/clawdsquad/` (published as `clawdsquad`).
- Shared schemas/routes: `packages/schema/` (`clawdsquad-schema`).
...
- Persist install state:
  - `./.clawdsquad/lock.json` (per workdir)
  - `./skills/<slug>/.clawdsquad/origin.json` (per skill folder)
