---
summary: 'Install telemetry collected via `clawdsquad sync` + opt-out.'
read_when:
...
ClawdSquad uses **minimal telemetry** to compute **install counts** (what’s actually in use) and to power better sorting/filtering.
This is based on the CLI `clawdsquad sync` command.
...
On each `clawdsquad sync`, the CLI reports a **full snapshot** of what it found, grouped by scan root (“folder/root”).
...
ClawdSquad provides a private “Installed” tab on your own profile:
...
With this set, the CLI will not send telemetry during `clawdsquad sync`.
