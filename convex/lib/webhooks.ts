...
const DEFAULT_SITE_URL = 'https://clawdsquad.com'
...
function buildDescription(event: WebhookEvent, skill: WebhookSkillPayload) {
  ...
  if (event === 'skill.highlighted') return 'Newly highlighted skill on ClawdSquad.'
  if (skill.version) return `New version v${skill.version} published on ClawdSquad.`
  return 'New skill published on ClawdSquad.'
}
